...
import os
import objectCreator.objectCreatorUtil as objutil
import importlib
...
class ObjectCreatorDialog(QtWidgets.QDialog):
	def __init__(self, parent=None):
		...
		self.create_button.clicked.connect(self.onClickObject)
	...
	def onClickObject(self):
		sel_item = self.object_listWidget.selectedItems()
		
		if not sel_item:
			return

		sel_text = sel_item[0].text()
		...
		if sel_text == 'cone':
			objutil.createCone(prefix, name, num)
		elif sel_text == 'cube':
			objutil.createCube(prefix, name, num)
		elif sel_text == 'sphere':
			objutil.createSphere(prefix, name, num)
		elif sel_text == 'torus':
			objutil.createTorus(prefix, name, num)
		elif sel_text == 'cylinder':
			objutil.createCylinder(prefix, name, num)
